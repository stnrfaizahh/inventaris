    ...
    public function exportPdf(Request $request)
{
    ...
    // ...kode perhitungan sama seperti index(), bisa refactor ke fungsi terpisah
    // Untuk efisiensi, copy logika yang sama dari index()

    // Ambil data inventaris (kode sama dengan `index()`)
    $lokasiList = Lokasi::all();
    $kategoriList = KategoriBarang::all();
    $barangList = Barang::with('kategori')
    ...
    foreach ($barangList as $barang) {
        ...
        foreach ($lokasiTarget as $lokasi) {
            ...
            $hilangManual = Hilang::where('id_barang', $barang->id_barang)
                ->where('id_lokasi', $lokasi->id)
                ->sum('jumlah_hilang');
    ...
    $pdf = Pdf::loadView('inventaris.export-pdf', compact('data'));
